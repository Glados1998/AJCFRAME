      ...
      *****************************************************************
       ...
       01  ENR-REAPPRO.
           ...
           05 RQUANTITEACOMMANDER    PIC         9(2).
           05 JOUR                   PIC         9(2).
           05 MOIS                   PIC         9(2).
           05 ANNEE                  PIC         9(4).
           05 FILLER                 PIC         X(2).
      *****************************************************************
       ...
       77 MAJCLIENTOK              PIC  X(1) VALUE 'N'.
      ...
      *    READ MJBASE
           ...
           IF FS-VENTESAS = 0 AND FS-VENTESEU = 0
      ...
      *          DISPLAY "FILE CHNGFILE OPENED"
             WHEN 23
                DISPLAY "FILE FSSTOCKS NONEXISTENT "
                MOVE 'ANOMALY' TO AL-MESSAGE-TYPE
                MOVE 12 TO AL-RETURN-CODE
                MOVE "FILE CHNGFILE NONEXISTENT " TO AL-DESCRIPTION
                CALL 'ANOMLOG' USING ANOMLOG-PARM-BLOCK END-CALL
             WHEN OTHER
                DISPLAY 'ERR READ FILE FSTOCKS ' FS-STOCKS
                MOVE 'ANOMALY' TO AL-MESSAGE-TYPE
                MOVE 12 TO AL-RETURN-CODE
      ...
      *      IF CONTINUEINSERT = 'Y'
             IF PRIXSTR NOT EQUAL TO '000.00'
              EXEC SQL
      ...
      * AUGMENTATION SOLDE CLIENT
            IF CONTINUEINSERT = 'Y'
               ...
               MOVE ITEMS-PRICE TO PRIXPRODUITENCOURS
      ...
      *          DISPLAY "EUROPE:RECUPERATION DU PRIX EN BASE"
            ...
            END-IF
             ...
             DISPLAY "ITEMS" ITEMS-O-NO ";" ITEMS-P-NO
             DISPLAY ITEMS-QUANTITY ";" PRIXSTR
      *     IF CONTINUEINSERT = 'Y'
            IF PRIXSTR NOT EQUAL TO '000.00'
                 EXEC SQL
                    INSERT INTO API3.ITEMS VALUES
      ...
      *    MODIF STOCKS APRES COMMANDE
           ...
           DISPLAY "CONTINUEUPDATE " CONTINUEINSERT
           IF CONTINUEINSERT = 'Y'
               OPEN INPUT STOCKS
               PERFORM MANAGEREAPPRO UNTIL WS-EOF-STOCKS = 'Y'
               MOVE 'N' TO WS-EOF-STOCKS
               CLOSE STOCKS
             IF FS-STOCKS NOT EQUAL ZERO THEN
                DISPLAY 'ERR CLOSE STOCKS : ' FS-STOCKS
                MOVE 'ANOMALY' TO AL-MESSAGE-TYPE
      ...
      *      DISPLAY "SEUILREAPPRO" SSEUILREAPPRO
            IF IDPRODUITENCOURS = SNUMPRODUIT
             ...
             DISPLAY "ECRITURE FICHIER REAPPRO " IDPRODUITENCOURS
      ...
      *    FD  REAPPRO
                    MOVE 'N' TO DEJAECRIT
                    PERFORM EXISTEREAPPRO
                    IF DEJAECRIT = 'N'
                        DISPLAY SNUMPRODUIT ";" SQUANTITEACOMMANDER
      *                 MOVE SNUMPRODUIT TO RNUMPRODUIT
                        DISPLAY "NUMPRODUIT" RNUMPRODUIT
      *                 MOVE SQUANTITEACOMMANDER TO RQUANTITEACOMMANDER
                        DISPLAY "QUANcom: " RQUANTITEACOMMANDER         DER
                        PERFORM WRITEREAPPRO
                    END-IF
             END-IF
             END-IF.

       WRITEREAPPRO.
           DISPLAY "writereappro"
           OPEN EXTEND REAPPRO
           MOVE SNUMPRODUIT TO RNUMPRODUIT
           MOVE SQUANTITEACOMMANDER TO RQUANTITEACOMMANDER
           PERFORM DATESYSTEME
       ...
       DATESYSTEME.
           ...
           DISPLAY "DATESYSTEME"
           MOVE WS-CURRENT-DAY TO JOUR
           MOVE WS-CURRENT-MONTH TO MOIS
           MOVE WS-CURRENT-YEAR TO ANNEE
           DISPLAY "JOUR:" JOUR "MOIS:" MOIS "ANNEE" ANNEE.

       EXISTEREAPPRO.

           OPEN INPUT REAPPRO
           PERFORM UNTIL WS-EOF-REAPPRO  = 'Y'
           READ REAPPRO
                AT END MOVE 'Y' TO WS-EOF-REAPPRO
           END-READ
           IF NOT WS-EOF-REAPPRO = 'Y'
            DISPLAY RNUMPRODUIT WS-EOF-REAPPRO
            IF RNUMPRODUIT = IDPRODUITENCOURS
                   MOVE 'Y' TO DEJAECRIT
      *            DISPLAY "OK"
            END-IF
           END-IF
      ...
      *                DISPLAY "0K; " PRODUCTS-P-NO ";" IDPRODUITENCOURS
                       IF PRODUCTS-P-NO = IDPRODUITENCOURS
                           MOVE PRODUCTS-PRICE TO PRIXCONV
                           MOVE PRIXCONV TO PRIXSTR
      ...
      *    ADD SOLDECOMMANDE TO BALANCE
           DISPLAY PRIXPRODUITENCOURS ";" QCPRODUITENCOURS
           COMPUTE SOLDECOMMANDE = PRIXPRODUITENCOURS * QCPRODUITENCOURS
           ADD SOLDECOMMANDE TO BALANCE
